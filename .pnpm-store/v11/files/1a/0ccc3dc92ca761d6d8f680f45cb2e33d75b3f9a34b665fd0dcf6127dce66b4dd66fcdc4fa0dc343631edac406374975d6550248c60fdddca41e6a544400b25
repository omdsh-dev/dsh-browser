import { CordisDynamicPackageId, CordisDynamicPluginId, HOST_BUILTIN_INSPECTION } from "@deepseek-ai/dsh-cordis-host-runner";
import { createUserMessage } from "@deepseek-ai/dsh-llm";
import { defineTool } from "@deepseek-ai/dsh-tools";
//#region lib/types/api-catalog.js
/**
...
const SERVICE_API = [
	{
		...
		methods: [{
			...
			parameters: [{
				name: "next",
				description: "resolved selection accepted by an entry point."
			}],
		...
		methods: [
			...
			},
			{
				...
				parameters: [
					{
						name: "id",
						description: "shared agent/session identity."
					},
					{
						name: "options",
						description: "concrete loop options."
					},
					{
						name: "meta",
						description: "optional fresh-session workspace metadata."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "ownerCtx",
					description: "caller context that structurally owns the lifecycle."
				}, {
					name: "options",
					description: "identities, optional live parent, session seed/metadata, loop options, setup, and cancellation."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "ownerCtx",
					description: "caller context that owns load, setup, and the live lifecycle."
				}, {
					name: "options",
					description: "persisted identity, optional live parent, loop options, setup, and cancellation."
				}],
				returns: "the published handle."
			}
		]
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "id",
					description: "the preset id, or `undefined` for {@link defaultId}."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "agentCtx",
					description: "the agent's scope context."
				}, {
					name: "id",
					description: "the preset id, or `undefined` for {@link defaultId}."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "agentCtx",
					description: "the joining agent's scope context."
				}, {
					name: "parentCtx",
					description: "the scope context of the agent whose composition to join."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "from",
						description: "the preset the copy starts from; shipped presets are the primary source, so any trust is accepted."
					},
					{
						name: "id",
						description: "the new preset's id, which becomes its directory name."
					},
					{
						name: "name",
						description: "display name for the copy; absent falls back to the id."
					}
				],
			...
			},
			{
				...
				parameters: [
					{
						name: "from",
						description: "the source preset id."
					},
					{
						name: "id",
						description: "the new preset id."
					},
					{
						name: "name",
						description: "the copy's optional display name."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "agent",
					description: "the agent whose composition to look inside."
				}, {
					name: "name",
					description: "the service name as the preset's rows resolve it."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "id",
					description: "the preset to compose the agent from instead."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "agent",
					description: "the session's live agent, resolved from the wire identity."
				}, {
					name: "agentPreset",
					description: "the preset to compose the agent from instead."
				}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "agent",
					description: "initiating Agent to inherit; presence is neither liveness proof nor authorization."
				}, {
					name: "operation",
					description: "synchronous or asynchronous operation to invoke."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "operation",
					description: "synchronous or asynchronous operation to invoke without an initiator."
				}],
				returns: "the exact value returned by `operation`.",
				throws: ["when the initiator scope is closing/disposed, or when `operation` throws."]
			},
			{
				...
				parameters: [{
					name: "factory",
					description: "the loop-owned factory {@link create}/{@link resume} delegate to."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "options",
					description: "shared identity, optional live parent, session seed/metadata, and agent options."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "options",
					description: "persisted identity, optional live parent, configuration, and setup."
				}],
				returns: "the handle after setup, rollback-covered publication, and loop start complete."
			},
			{
				...
				parameters: [{
					name: "agent",
					description: "the already-constructed agent to record in the store."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "agent",
					description: "the prepared, unpublished agent."
				}, {
					name: "owner",
					description: "explicitly supplied live runtime owner, or undefined for a top-level runtime root. This is runtime ownership, not the resumed session's durable parent lineage."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "agent",
					description: "the live inserted agent to announce."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "id",
					description: "the shared agent/session id to look up."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "id",
					description: "the candidate child agent's shared agent/session id."
				}, {
					name: "owner",
					description: "the expected runtime creator agent."
				}],
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					...
					name: "request",
					description: "target name, content, and pre-queue cancellation."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "agent",
					description: "the live agent whose policy is changing."
				}, {
					name: "policy",
					description: "the new effective policy."
				}]
			},
			{
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "session whose log supplies the override."
				}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "content",
					description: "prompt parts in message order after file receipt resolution."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "input",
					description: "canonical base64 bytes and optional display name."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "error",
					description: "value caught from an attachment operation."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "ref",
					description: "durable reference from the session log."
				}, {
					name: "signal",
			...
			},
			{
				...
				parameters: [{
					name: "ref",
					description: "durable normalized attachment reference."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "input",
					description: "exact bytes and optional display name."
				}],
				returns: "the durable content-addressed file reference."
			},
			{
				...
				parameters: [{
					name: "input",
					description: "ordered exact bytes, optional cancellation, and display name."
				}],
				returns: "the durable content-addressed file reference."
			},
			{
				...
				parameters: [{
					...
					name: "signal",
					description: "optional cancellation for backend reads and verification work."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "ref",
					description: "durable file reference."
				}],
				returns: "an absolute host path, or undefined when this backend is not host-file-backed.",
				throws: ["an AttachmentError when the durable reference is invalid."]
			},
			{
				...
				parameters: [
					...
					{
						name: "policy",
						description: "exact route pixel budget and encoded-byte target; a target no ladder quality meets yields the smallest ladder output."
					},
					{
						name: "signal",
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "shell-carrier request for a `/plugins` resource."
				}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "the program, its bindings, and the abort signal; the request carries everything the runtime acts on, with no hidden defaults."
				}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "resolver",
					description: "Session-aware receipt resolver."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "name",
					description: "command name without a slash."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "agent",
						description: "exact receiving agent."
					},
					{
						name: "line",
					...
					},
					{
						...
						description: "encoded images and staged file receipts accompanying the line, in submission order; empty for a plain invocation."
					},
					{
						name: "signal",
		...
		methods: [
			{
				...
				parameters: [
					{
						name: "agent",
						description: "agent context owning the session surface and routing options."
					},
					{
						name: "trigger",
						description: "normal pressure or provider-confirmed context overflow."
					},
					{
						name: "signal",
						description: "cancellation signal; model-backed implementations must forward it."
					}
				],
			...
			},
			{
				...
				parameters: [
					{
						name: "agent",
						description: "idle agent whose durable history should be compacted."
					},
					{
						name: "signal",
						description: "cancellation scoped to this compaction request."
					},
					{
						name: "sourceCommandId",
						description: "initiating command identity for a manual compaction."
					}
				],
			...
			},
			{
				...
				parameters: [
					{
						name: "start",
						description: "first surface seq, inclusive."
					},
					{
						name: "end",
						description: "last surface seq, inclusive."
					},
					{
						name: "agent",
						description: "context whose session is mutated and whose routing options guide summarization."
					},
					{
						name: "signal",
						description: "optional cancellation; model-backed implementations must forward it."
					}
				],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "ref",
					description: "the reference to describe."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "ref",
					description: "the reference to store."
				}, {
					name: "value",
		...
		methods: [
			{
				...
				parameters: [{
					name: "refs",
					description: "reference names, at most {@link MAX_DESCRIBE_REFS}; a name outside the grammar rejects the whole call as `gateway/bad-request`."
				}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "field",
				description: "declaration-merged field owned by the provider."
			}, {
				name: "provider",
				description: "request-time field preparation and optional acceptance behavior."
			}],
			...
			parameters: [{
				name: "request",
				description: "exact serialized request facts before extension fields."
			}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "signal",
					description: "caller lifetime; abort terminates the chooser."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "path",
					description: "absolute directory to list; absent lists the home directory."
				}, {
					name: "signal",
					description: "caller lifetime; abort stops the backend's scan instead of letting it outlive a disconnected caller."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "resolve",
					description: "resolver that returns the exact live Agent or throws a Remote error."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "agent",
						description: "receiving Agent resolved from the Remote Agent scope."
					},
					{
						name: "request",
						description: "canonical base64 bytes and optional display name."
					...
					{
						name: "signal",
						description: "caller cancellation before storage begins."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "Session identity, ordered bytes, cancellation, and optional display name."
				}],
				returns: "the staged receipt and durable file reference."
			},
			{
				...
				parameters: [{
					...
					name: "receiptId",
					description: "opaque receipt minted for one completed upload."
				}],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "receiptIds",
						description: "distinct staged receipts referenced by the prompt."
					},
					{
						name: "requestId",
						description: "prompt identity later observed in queue or history."
					}
				],
			...
			},
			{
				...
				parameters: [{
					...
					name: "requestId",
					description: "prompt identity carried by the queue occurrence."
				}]
			}
		]
	},
		...
		methods: [
			{
				...
				parameters: [{
					name: "path",
					description: "the path to resolve; relative paths resolve against `opts.cwd`."
				}, {
					name: "opts",
					description: "optional cwd override and cancellation signal."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "target",
					description: "the resolved target whose process path is required."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "hostPath",
					description: "absolute path in the harness host filesystem."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "target",
					description: "the resolved target to encode."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "parent",
					description: "canonical directory target."
				}, {
					name: "child",
					description: "canonical candidate target."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "target",
					description: "the resolved target to stat."
				}, {
					name: "signal",
					description: "aborts the metadata round-trip."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "path",
						description: "the path to inspect; relative paths resolve against `opts.cwd`."
					},
					{
						name: "opts",
						description: "`cwd` overrides the backend's default base for relative paths."
					},
					{
						name: "signal",
						description: "aborts the metadata round-trip."
			...
			},
			{
				...
				parameters: [{
					name: "target",
					description: "the resolved target to read."
				}, {
					name: "signal",
					description: "aborts the read."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "signal",
					description: "aborts the stream, including between chunks."
				}],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "maxBytes",
						description: "inclusive byte cap on the complete content."
					}
				],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "range",
						description: "`offset`, the 0-based first byte, and `length`, the largest byte count; both non-negative integers."
					},
					{
						name: "signal",
						description: "aborts the read."
			...
			},
			{
				...
				parameters: [{
					name: "target",
					description: "the resolved directory target."
				}, {
					name: "signal",
					description: "aborts the listing."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "target",
						description: "the resolved target to write."
					},
					{
						name: "content",
					...
					{
						name: "expected",
						description: "the write intent guarding the write; omit for unconditional."
					},
					{
						name: "signal",
						description: "aborts before atomic publication takes effect."
					},
					{
						name: "sandboxPolicy",
						description: "the per-call mode and workspace root this write runs under; a sandboxing backend fences the write by it, the bare backend ignores it. Omit to leave the backend its own default."
					}
				],
			...
			},
			{
				...
				parameters: [
					{
						name: "target",
						description: "the resolved target to edit."
					},
					{
						name: "edit",
						description: "the literal search/replace request."
					},
					{
						name: "expected",
						description: "the version guard; omit for an unconditional edit."
					},
					{
						name: "signal",
						description: "aborts before atomic publication takes effect."
					...
					{
						name: "sandboxPolicy",
						description: "the per-call mode and workspace root this edit runs under; a sandboxing backend fences the edit by it, the bare backend ignores it. Omit to leave the backend its own default."
					}
				],
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					...
					name: "request",
					description: "objective and optional round cap."
				}],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "request",
						description: "at least one replacement field."
					}
				],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "reason",
						description: "policy-owned stable code and human-readable explanation."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "agent",
					description: "exact live Agent resolved from the wire identity."
				}, {
					name: "request",
					description: "objective and optional round cap."
				}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [
				{
					name: "topic",
					description: "Domain-owned topic name."
				},
				{
					name: "payload",
					description: "JSON value validated before it reaches the carrier."
				},
				{
					name: "monotonicMs",
					description: "Source-clock timestamp; defaults to `performance.now()`."
				}
			]
		}, {
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "packageName",
				description: "full npm package name that owns the contribution."
			}, {
				name: "installer",
				description: "listener or startup-check installer for the child context."
			}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "spec",
					description: "job identity, owner, and synchronous starter."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "caller",
					description: "reading agent; a non-agent caller sees only unowned jobs."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "id",
					description: "job to look up."
				}, {
					name: "caller",
					description: "reading agent checked against the owner."
				}],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "caller",
						description: "killing agent checked against the owner."
					},
					{
						name: "reason",
						description: "logged reason forwarded to the producer."
					}
				],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "timeoutMs",
						description: "positive finite wait bound in milliseconds."
					},
					{
						name: "caller",
						description: "waiting agent checked against the owner."
					},
					{
						name: "signal",
						description: "optional cancellation of the wait itself."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "listener",
					description: "receives each terminal snapshot and its exact owner."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "listener",
					description: "receives the owner whose visible set changed, or `undefined` when an unowned job changed and every caller's set did."
				}],
				returns: "disposer that unregisters the listener."
			},
			{
				...
				parameters: [{
					name: "name",
					description: "diagnostic label; duplicate names remain independent."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "providers",
					description: "every provider route this adapter should serve."
				}, {
					name: "adapter",
					description: "the adapter that streams calls for those providers."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "entries",
					description: "every configurable provider this plugin owns."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "settingsNs",
					description: "the namespace whose profiles this discovery serves."
				}, {
					name: "discover",
					description: "interrogates one endpoint and must honor the supplied signal."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "settingsNs",
						description: "namespace whose registered discovery serves this draft."
					},
					{
						name: "request",
						description: "the endpoint, protocol, and one-shot credential to use."
					},
					{
						name: "signal",
						description: "caller cancellation."
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "request",
						description: "endpoint, protocol, and one-shot credential to use."
					},
					{
						name: "signal",
						description: "caller cancellation supplied by the Remote carrier."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "provider",
					description: "registered provider route to inspect."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "provider",
					description: "provider route named by a request header."
				}, {
					name: "model",
					description: "exact model id named by the same header."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "ref",
					description: "durable verbatim file reference from model history."
				}],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "model",
						description: "exact model id passed to the adapter."
					},
					{
						name: "signal",
						description: "optional cancellation for adapter-owned asynchronous lookup."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "config",
					description: "provider/model route and optional request controls."
				}, {
					name: "signal",
					description: "optional cancellation for adapter-owned capability lookup."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "options",
					description: "the full request; `options.provider` selects the adapter."
				}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "provider",
				description: "the backend to register."
			}],
			...
			parameters: [{
				...
				name: "signal",
				description: "optional cancellation forwarded to the selected provider."
			}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "Target, desired value, and observed item version."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "Session, message, and observed item version."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "session",
					description: "the session whose knob state is read."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "state",
					description: "the folded knob overrides."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "name",
					description: "the preset name to resolve."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "name",
					description: "a table key, or `custom`."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "the session the switch belongs to."
				}, {
					name: "name",
					description: "the preset to switch to; unknown names throw."
				}]
			}
		]
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "agent",
				description: "The agent to switch."
			}, {
				name: "active",
				description: "Whether plan mode should be active."
			}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "argv",
				description: "the exact argv the caller is about to spawn (program plus arguments), NOT a shell string — a shell-shaped consumer passes `['bash', '-c', command]`."
			}, {
				name: "policy",
				description: "the file-effect policy this execution runs under, carried per call (see {@link SandboxPolicy})."
			}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "optional session and approved mode override."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "sessionId",
					description: "Session identity whose Agent owns the operation."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "sessionId",
					description: "durable Session identity."
				}, {
					name: "signal",
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "path after best-effort Session workspace resolution."
				}, {
					name: "signal",
					description: "caller lifetime; abort terminates the native command."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "durable address, backward cursor, and page budget."
				}, {
					name: "signal",
					description: "cancellation for persistence reads."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "durable address and last committed sequence already held by the caller."
				}, {
					name: "signal",
					description: "cancellation owned by the Remote stream carrier."
				}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "request",
				description: "target Session plus the optional text and category."
			}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "header",
					description: "the immutable header (id, version, cwd, lineage) to store."
				}, {
					name: "options",
					description: "optional cancellation."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "id",
						description: "the stored session to open."
					},
					{
						name: "access",
			...
			},
			{
				...
				parameters: [{
					name: "id",
					description: "the stored session to observe."
				}, {
					name: "options",
					description: "optional cancellation."
				}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "meta",
					description: "authoritative listed Session header."
				}, {
					name: "inheritedEventCount",
					description: "exact inherited cut completing the lifecycle identity."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "the live session to checkpoint."
				}],
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					...
					name: "keys",
					description: "optional client-visible outputs; state materialization remains complete."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "attached Session whose cached cells are inspected."
				}, {
					name: "keys",
					description: "optional wire keys to view."
				}],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "events",
						description: "the stored events with `seq >= baseSeq`, in seq order."
					},
					{
						name: "baseSeq",
						description: "the seq `events` starts at (its first event's seq when non-empty)."
					},
					{
						name: "header",
			...
			},
			{
				...
				parameters: [
					{
						name: "session",
						description: "exact prepared Session that owns the restored log prefix."
					},
					{
						name: "checkpoint",
						description: "persisted rows for this Session lifecycle."
					},
					{
						name: "events",
						description: "exact events at the observation cut."
					},
					{
						name: "baseSeq",
						description: "first supplied event sequence."
					}
				],
		...
		methods: [
			{
				...
				parameters: [{
					name: "sessionId",
					description: "logical Session identity."
				}, {
					name: "options",
					description: "cancellation and projection selection for this read."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "query text, metadata filters, page size, and cursor."
				}, {
					name: "exec",
					description: "optional cancellation control."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "target session, query text, filters, page size, and cursor."
				}, {
					name: "exec",
					description: "optional cancellation control."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "signal",
					description: "optional cancellation for persistence listing."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "sessionId",
					description: "live or persisted session id to read."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "filters",
					description: "ANDed session metadata and availability clauses."
				}, {
					name: "signal",
					description: "optional cancellation for persistence listing."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "signal",
					description: "optional cancellation for source resolution and title folding."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "sessionIds",
					description: "live or persisted session ids to observe."
				}, {
					name: "signal",
					description: "optional cancellation shared by all source reads."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "sessionId",
					description: "live-preferred session id to read."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "sessionId",
					description: "live-preferred session id to scan."
				}, {
					name: "filters",
					description: "ANDed metadata and literal-text predicates."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "sessionId",
					description: "logical session id to trace."
				}, {
					name: "signal",
					description: "optional cancellation for persistence listing."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "target session id and event seq."
				}, {
					name: "signal",
					description: "optional cancellation for persisted source resolution."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "target session/seq and context sizes."
				}, {
					name: "signal",
					description: "optional cancellation for persisted source resolution."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "id",
					description: "the session id; omitted, the store mints `session-<n>`."
				}, {
					name: "options",
					description: "seed events and/or creation metadata for the header."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "options",
					description: "seed events and/or creation metadata for the header. With `eventState`, every seed event is either independently owned or any shared value is deeply frozen; {@link Session.fromRestore} validates and adopts those values without copying or freezing them."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "a {@link prepare}d session not yet in the store."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "the entered session to announce to listeners."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "the session whose buffered events must reach durable storage."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "source",
						description: "Live source session object or id."
					},
					{
						name: "boundary",
						description: "Inclusive source event seq to fork through; omitted means the source's current last event, and omitted on an empty source forks an empty child."
					},
					{
						name: "childSessionId",
						description: "Optional child session id; omitted delegates to `SessionStore`'s id policy."
					}
				],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "request",
				description: "Session identity whose cwd and preset select the catalog view."
			}, {
				name: "signal",
				description: "caller lifetime carried by the Remote transport; admitted catalog reads retain their existing completion semantics."
			}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "record",
					description: "the logical record to report; owned by the backend after the call."
				}]
			},
			{
		...
		methods: [
			{
				...
				parameters: [{
					name: "session",
					description: "session whose log is the title source of truth."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "exact live session to rename."
				}, {
					name: "title",
					description: "raw user input; normalized before acceptance."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "exact live session to refresh."
				}, {
					name: "signal",
					description: "optional caller cancellation."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "provider",
					description: "provider identity, cadence, and generation function."
				}],
		...
		methods: [
			...
			},
			{
				...
				parameters: [
					{
						name: "ns",
						description: "unique namespace; duplicate registration fails loud."
					},
					{
						name: "schema",
						description: "schemastery schema resolving this namespace's value."
					},
					{
						name: "options",
						description: "composition `base` layer and effect timing."
					}
				],
			...
			},
			{
				...
				parameters: [
					{
						name: "owner",
						description: "consumer context whose unload suppresses fallback work."
					},
					{
						name: "ns",
						description: "consumer-owned settings namespace."
					},
					{
						name: "schema",
						description: "schema resolving the namespace."
					},
					{
						name: "entry",
						description: "composition entry used as the base and fallback value."
					},
					{
						name: "hooks",
						description: "source sink, change notification, and optional validation."
					}
				],
				throws: ["{TypeError} when `ns` is not a lowercase hyphenated identifier."]
			},
			{
				...
				parameters: [{
					name: "options",
					description: "redaction switch; wire surfaces must redact."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "ns",
					description: "the namespace to read."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "ns",
						description: "the registered namespace to update."
					},
					{
						name: "patch",
						description: "plain-object patch over the user section."
					},
					{
						name: "expectedRevision",
						description: "the descriptor `revision` the caller read; a namespace that moved past it rejects with {@link SettingsConflictError}."
					}
				],
				throws: ["{TypeError} when `ns` is not a lowercase hyphenated identifier."]
			},
			{
				...
				parameters: [
					{
						name: "ns",
						description: "the registered namespace to replace."
					},
					{
						name: "section",
						description: "the complete next user section."
					},
					{
						name: "expectedRevision",
						description: "the descriptor `revision` the caller read; a namespace that moved past it rejects with {@link SettingsConflictError}."
			...
			},
			{
				...
				parameters: [
					{
						name: "ns",
						description: "the registered namespace to edit."
					},
					{
						name: "ops",
						description: "ordered path edits; later ops observe earlier ones."
					},
					{
						name: "expectedRevision",
						description: "the descriptor `revision` the caller read; a namespace that moved past it rejects with {@link SettingsConflictError}."
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "signal",
					description: "caller lifetime; abort terminates preparation or the native command."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "agentPreset",
					description: "preset id resolved against Host-owned roots."
				}, {
					name: "signal",
					description: "caller lifetime; abort terminates the native command."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "request",
					description: "the caller's request; omitted fields get this implementation's defaults, capped fields are clamped."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "spec",
					description: "a resolved spec from {@link resolve}, never a raw request."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "contributor",
					description: "declared key ownership and per-execution resolver."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "execution",
					description: "the current tool execution."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "create",
					description: "synchronous factory receiving this registration's lifecycle and invalidation control."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "skill",
					description: "the skill definition input; omitted invocation and provider fields receive defaults."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "options",
					description: "view options; `scope` selects the viewing agent's layers, `cwd` selects project roots, and `signal` cancels discovery."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "options",
					description: "view options; `scope` selects the viewing agent's layers, `cwd` selects workspace-sensitive skills, and `signal` cancels work."
				}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "input",
				description: "the owner, caller-supplied source fields, suggested name, and full text to save."
			}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "form",
					description: "Form key declared in {@link StorageForms}."
				}, {
					name: "facility",
					description: "The facility instance to expose."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "spec",
					description: "The domain declaration, typically from `defineDomain`."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "spec",
					description: "provider, delegation request, and caller cancellation."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					description: "the durable child session id to interrupt."
				}, {
					name: "authority",
					description: "the human parent address or exact live ancestor Agent."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "parents",
					description: "exact host-owned parent Agents entering teardown."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					description: "parent session whose direct children are listed."
				}, {
					name: "signal",
					description: "caller-owned cancellation forwarded to Session queries and observed around every read await."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "rootSessionId",
					description: "session whose complete descendant tree is listed."
				}, {
					name: "signal",
					description: "caller-owned cancellation forwarded to persistence reads and observed around every read await."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "signal",
					description: "carrier cancellation forwarded to Session queries."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "durable address, delivery, minted identity, content, and optional browser zone."
				}, {
					name: "signal",
			...
			},
			{
				...
				parameters: [
					{
						name: "childSessionId",
						description: "durable child session id to interrupt."
					},
					{
						name: "parentSessionId",
						description: "durable direct parent whose authority is claimed."
					},
					{
						name: "mode",
						description: "required continuable-address discriminator."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "provider",
					description: "the trusted provider implementation."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "request",
					description: "child label, prompt, parent, signal, and optional capabilities."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "section",
					description: "the section to register."
				}],
				returns: "the exact Cordis effect disposer."
			},
			{
				...
				parameters: [{
					name: "name",
					description: "stable section placement name."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "name",
					description: "stable context placement name."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "provider",
					description: "evaluated for each assembly."
				}],
				returns: "the exact Cordis effect disposer."
			},
			{
				...
				parameters: [{
					name: "context",
					description: "the optional scope and plugin-defined assembly fields."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "backend",
					description: "provider with a non-empty unique type."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "owner",
						description: "exact registered Agent that owns access and cleanup."
					},
					{
						name: "request",
						description: "backend type plus optional owner-local name and cwd."
					},
					{
						name: "signal",
						description: "cancellation of unpublished setup."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "owner",
					description: "exact live owner to inspect."
				}],
			...
			},
			{
				...
				parameters: [
					{
						name: "owner",
						description: "exact session owner."
					},
					{
						name: "id",
						description: "target PTY identity."
					},
					{
						name: "request",
						description: "explicit text, submit behavior, and cancellation."
					}
				],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "request",
						description: "optional newest-relative offset and line count."
					}
				],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "signal",
						description: "allowed POSIX signal name."
					}
				],
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "reason",
						description: "diagnostic cleanup reason."
					}
				],
			...
			},
			{
				...
				parameters: [{
					name: "owner",
					description: "exact owner whose sessions are visible."
				}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "session",
				description: "session to replay through its current durable tail."
			}, {
				name: "requestHeader",
				description: "optional effective request envelope replacing the latest logged header."
			}],
			...
			parameters: [{
				name: "message",
				description: "message to price without mutation."
			}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "blocks",
					description: "tool-result content to measure."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "blocks",
					description: "original tool-result content."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "session",
					description: "session whose current surface is rewritten."
				}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "contribution",
					description: "generated schemas, reflection, and Host invocations."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "filter",
					description: "optional package and face restriction."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "face",
					description: "face to query; defaults to the host runtime."
				}],
			...
			},
			{
				...
				parameters: [{
					...
					name: "params",
					description: "Zod projection parameters."
				}],
	...
	},
	{
		...
		methods: [
			...
			},
			{
				...
				parameters: [{
					name: "source",
					description: "stream factory installed by the Remote assembly."
				}, {
					name: "host",
					description: "stable Host facts included in each Client generation's opening frame."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "decoded endpoint and exact named wire arguments."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "decoded endpoint and named wire arguments."
				}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "request",
				description: "Questions, owner agent, and abort signal."
			}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "provider",
					description: "the provider; its `id` is the registry key."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "the query and optional result limit."
				}, {
					name: "signal",
					description: "optional cancellation signal forwarded to the provider."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "request",
					description: "the URL plus retrieval options."
				}, {
					name: "signal",
					description: "optional cancellation signal forwarded to the provider."
				}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "rule",
				description: "unique id, provider kind, and arbitrary callback."
			}],
			...
			parameters: [{
				name: "delivery",
				description: "authenticated provider data; snapshotted before dispatch."
			}],
		...
		methods: [
			{
				...
				parameters: [{
					name: "route",
					description: "kind, path, and the owning handler."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "route",
					description: "pathname and handler owning negotiation plus socket use."
				}],
				returns: "the disposer removing the route."
			},
			{
				...
				parameters: [{
					name: "handler",
					description: "owns the full response lifecycle of unmatched requests."
				}],
	...
	},
	{
		...
		methods: [{
			...
			parameters: [{
				name: "request",
				description: "the script, its `args`, the parent agent, and an optional cancel signal."
			}],
		...
		methods: [
				...
				parameters: [
					{
						...
						description: "header-derived workspace root for the Session identity on the wire."
					},
					{
						name: "path",
						description: "absolute path or path relative to the workspace root; files outside it are allowed."
					},
					{
						name: "range",
						description: "the line window; omitted fields take the page defaults."
					},
					{
						name: "signal",
						description: "caller cancellation."
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "range",
						description: "the byte window; omitted fields take the window defaults."
					},
					{
						name: "signal",
						description: "caller cancellation."
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "path",
						description: "base file, absolute or workspace-relative."
					},
					{
						name: "relativePath",
						description: "relative filesystem path, not a URL or absolute path."
					},
					{
						name: "signal",
						description: "caller cancellation."
			...
			},
			{
				...
				parameters: [
					...
					{
						name: "path",
						description: "workspace path, absolute or relative to the workspace root."
					},
					{
						name: "signal",
						description: "caller cancellation."
		...
		methods: [
			{
				...
				parameters: [{
					name: "path",
					description: "Existing directory to own, in a fully qualified path spelling."
				}, {
					name: "title",
					description: "Display title used only when a new record is created."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "id",
					description: "Workspace registration to remove."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "id",
					description: "Workspace to move."
				}, {
					name: "beforeId",
					description: "Workspace anchor; omitted appends."
				}],
			...
			},
			{
				...
				parameters: [{
					name: "path",
					description: "Existing directory path in a fully qualified spelling."
				}],
...
const EVENT_API = [
	{
		...
		parameters: [{
			name: "payload",
			description: ".error - persistence, setup, or publication failure."
		}]
	},
	{
		...
		parameters: [{
			name: "sessionId",
			description: "the session whose composition changed."
		}, {
			name: "agentPreset",
			description: "the preset recorded by the committed selection."
		}]
	},
	{
		...
		parameters: [{
			name: "payload",
			description: ".frame - one ordered start, chunk, or end publication. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		name: "agent/created",
		...
		parameters: [{
			name: "payload",
			description: ".agent - the newly registered agent with its live session and completed setup. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		name: "agent/disposed",
		...
		parameters: [{
			name: "payload",
			description: ".agent - the exact agent removed from the registry. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		name: "agent/error",
		...
		parameters: [{
			name: "payload",
			description: ".error - the failure, verbatim. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		...
		parameters: [{
			name: "payload",
			description: ".turn - the owning turn. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		...
		parameters: [{
			name: "payload",
			description: ".message - the discarded message. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		...
		parameters: [{
			name: "payload",
			description: ".message - the inserted message. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		name: "agent/pre-step",
		...
		parameters: [{
			name: "payload",
			description: ".signal - the current turn's cancellation signal. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		name: "agent/request",
		...
		parameters: [{
			name: "payload",
			description: ".signal - the current turn's explicit abort signal. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		...
		parameters: [{
			name: "payload",
			description: ".signal - the turn abort signal. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		...
		parameters: [{
			name: "payload",
			description: ".source - why the session started (fresh startup, resume, …). Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
		name: "agent/status",
		...
		parameters: [{
			name: "payload",
			description: ".status - the status just entered (the transition's destination). Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent."
		}]
	},
	{
	...
	},
	{
		...
		parameters: [{
			name: "sessionId",
			description: "addressed Session identity."
		}, {
			name: "updatedAt",
			description: "durable message time used for list ordering."
		}]
	},
	{
		...
		parameters: [{
			name: "summary",
			description: "initial list row for the Session."
		}]
	},
	{
		...
		parameters: [{
			name: "sessionId",
			description: "Agent and Session identity."
		}, {
			name: "message",
			description: "user-safe failure chain."
		}]
	},
	{
		...
		parameters: [{
			name: "sessionId",
			description: "removed Session identity."
		}]
	},
	{
		...
		parameters: [{
			...
			name: "running",
			description: "whether the Agent is running."
		}]
	},
	{
	...
	},
	{
		...
		parameters: [{
			name: "pkg",
			description: "stable plugin, immutable package, run identity, and label."
		}]
	},
	{
		...
		parameters: [{
			name: "retracted",
			description: "plugin, package, and run identity."
		}]
	},
	{
		...
		parameters: [{
			name: "request",
			description: "correlation, Session, provider, method, and JSON input."
		}]
	},
	{
		...
		parameters: [{
			name: "resolved",
			description: "exact query identity that is no longer answerable."
		}]
	},
	{
		...
		parameters: [{
			name: "request",
			description: "correlation identity, owner, target version, mode, and approval requirement."
		}]
	},
	{
		...
		parameters: [{
			name: "resolved",
			description: "request identity and outcome."
		}]
	},
	{
	...
	},
	{
		...
		parameters: [{
			name: "ref",
			description: "the reference whose stored value changed."
		}]
	},
	{
		name: "domain/changed",
		...
		parameters: [{
			name: "change",
			description: "domain, table (`''` for global), key (`''` for global), operation discriminant, and on `put` the new snapshot."
		}]
	},
	{
		...
		parameters: [{
			name: "inspection",
			description: "committed canonical prefix, including the feedback as its last event."
		}]
	},
	{
		name: "fs/edit-intent",
		...
		parameters: [{
			name: "target",
			description: "the resolved target about to be edited."
		}, {
			name: "actor",
			description: "the opaque tool-execution context the decider keys off."
		}]
	},
	{
		name: "fs/observed",
		...
		parameters: [
			{
				name: "target",
				description: "the target whose presence or absence was observed."
			},
			{
				name: "observation",
				description: "present with its version, or confirmed absent."
			},
			{
				name: "actor",
				description: "the observing tool-execution context; undefined records nothing useful."
			}
		]
	},
	{
		...
		parameters: [{
			name: "target",
			description: "the resolved target about to be written."
		}, {
			name: "actor",
			description: "the opaque tool-execution context the decider keys off."
		}]
	},
	{
		...
		parameters: [{
			name: "payload",
			description: "session id and the exact current goal activation, or no goal after a clear."
		}]
	},
	{
		name: "goal/changed",
		...
		parameters: [{
			name: "payload",
			description: ".change - fresh current projection or clear tombstone."
		}]
	},
	{
	...
	},
	{
		...
		parameters: [{
			name: "options",
			description: "the full request. A LOOP-built request carries the process-local {@link markAgentLoopRequest} identity and arrives deep-frozen (mutation throws): its content is a pure function of the session log (the reconstructability Agent Note), so listeners read it, never rewrite it. Hand-built calls do not carry that marker; their messages already obey the immutable creation contract."
		}]
	},
	{
		...
		parameters: [{
			name: "record",
			description: "the candidate record, already the coordinator's own deep copy; listeners return a (possibly new) record and must not mutate it."
		}]
	},
	{
		...
		parameters: [{
			name: "session",
			description: "the session just entered and announced."
		}]
	},
	{
		...
		parameters: [{
			name: "session",
			description: "the session that is no longer live in the store."
		}]
	},
	{
		name: "session/event",
		...
		parameters: [{
			name: "session",
			description: "the session whose log grew."
		}, {
			name: "event",
			description: "the appended event, exactly as recorded."
		}]
	},
	{
		name: "session/flush",
	...
	},
	{
		...
		parameters: [{
			name: "ns",
			description: "the namespace whose stored section changed."
		}, {
			name: "revision",
			description: "the namespace's new revision."
		}]
	},
	{
		...
		parameters: [
			{
				name: "ns",
				description: "the namespace whose resolved value changed."
			},
			{
				name: "next",
				description: "the new resolved value."
			},
			{
				name: "prev",
				description: "the previous resolved value."
			},
			{
				name: "source",
				description: "whether the change entered through `update()` or the provider."
			}
		]
	},
	{
	...
	},
	{
		...
		parameters: [{
			name: "info",
			description: "the run identity and terminal outcome."
		}]
	},
	{
	...
	},
	{
		...
		parameters: [{
			name: "name",
			description: "the provider name that no longer resolves."
		}]
	},
	{
		name: "subagent/start",
		...
		parameters: [{
			name: "info",
			description: "the provider and published child identity."
		}]
	},
	{
		...
		parameters: [{
			name: "assembly",
			description: "the mutable assembly built from registered providers."
		}, {
			name: "context",
			description: "the caller's per-assembly context."
		}]
	},
	{
	...
	},
	{
		...
		parameters: [{
			name: "exec",
			description: "the allowed call about to dispatch (name, parsed arguments, caller agent, signal)."
		}]
	},
	{
		...
		parameters: [{
			name: "exec",
			description: "the call that just ran (name, parsed arguments, caller agent)."
		}, {
			name: "result",
			description: "the dispatch outcome a listener may accept, replace, or block."
		}]
	},
	{
	...
	},
	{
		...
		parameters: [{
			name: "dispatch",
			description: "the parent execution, sub-call identity, and the settled content to log."
		}]
	},
	{
		name: "tools/result",
		...
		parameters: [{
			name: "exec",
			description: "the execution object that traversed the pipeline."
		}, {
			name: "result",
	...
	},
	{
		...
		parameters: [{
			name: "request",
			description: "pending user-question request."
		}]
	},
	{
	...
	},
	{
		...
		parameters: [{
			name: "info",
			description: "the run's identity snapshot."
		}, {
			name: "agent",
			description: "the call identity plus its outcome."
		}]
	},
	{
		...
		parameters: [{
			...
			name: "agent",
			description: "the call's sequence number, label, phase, and child id."
		}]
	},
	{
		name: "workflow/end",
		...
		parameters: [{
			...
			name: "result",
			description: "the outcome data (stop reason, error, agent count) — deliberately WITHOUT the result value (see {@link WorkflowResultInfo})."
		}]
	},
	{
		name: "workflow/log",
		...
		parameters: [{
			...
			name: "message",
			description: "the logged message, verbatim."
		}]
	},
	{
		name: "workflow/phase",
		...
		parameters: [{
			...
			name: "title",
			description: "the phase title, verbatim."
		}]
	},
	{
		name: "workflow/start",
		...
		parameters: [{
			name: "info",
			description: "the run's identity snapshot (id + meta)."
		}]
	}
];
...
//#endregion
//#region lib/types/fiber-state.js
/**
...
FiberState.PENDING, FiberState.LOADING, FiberState.ACTIVE, FiberState.FAILED, FiberState.DISPOSED, FiberState.UNLOADING;
//#endregion
//#region lib/types/inspect.js
...
* @returns replay-safe generic call presentation.
*/
function presentInspectSelfCall(args) {
	return {
		card: "generic",
		kind: "read",
		title: `Inspect ${args.pluginId === void 0 ? "dynamic Cordis Plugins" : args.packageId === void 0 ? args.pluginId : `${args.pluginId}/${args.packageId}`}`
	};
}
/**
...
*/
function presentDefineCall(args) {
...
* @returns replay-safe generic call presentation.
*/
function presentUndefineCall(args) {
	return {
		card: "generic",
		kind: "delete",
		title: `Remove Cordis Plugin ${args.pluginId}`
	};
}
/**
...
* @returns replay-safe generic call presentation.
*/
function presentStopCall(args) {
	return {
		card: "generic",
		kind: "execute",
		title: `Stop Cordis Plugin ${args.pluginId}`
	};
}
//#endregion
//#region lib/types/prompt.js
/** Model guidance shared by the Cordis dynamic-plugin tools. */
const CORDIS_SYSTEM_PROMPT = `# Dynamic Cordis Plugins
...
*/
function hostInspectProviders(ctx) {
	return [
...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		execute(args, exec) {
			...
			if (args.pluginId === void 0) return Promise.resolve({
				mode: "plugins",
			...
			return Promise.resolve(inspectSelfPackage(ctx, agent, pluginId, CordisDynamicPackageId(args.packageId)));
		},
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => {
				...
				return [{
					type: "text",
					text: result.status === "awaiting-approval" ? `${pluginId}/${packageId} is awaiting user approval (${pluginRunId}).` : result.status === "starting" ? `${pluginId}/${packageId} is starting asynchronously (${pluginRunId}).` : `${pluginId}/${packageId} is running (${pluginRunId}).`
				}];
			},
			presentationMeta: (_args, value) => {
				const result = requireJsonObject(value);
	...
	ctx.on("agent/pre-step", async ({ agent, messages, signal }, next) => {
		...
		const contexts = ids.map((id) => {
			...
			return createUserMessage({
				content: [{
					type: "text",
					text: reference === void 0 ? renderUnavailableReference(id) : renderReference(reference)
				}],
				source: {
					kind: "plugin",
					plugin: name,
		...
		return {
			...decision,
			messages: [...decision.messages, ...contexts]
		};
	});
}
...
function referencedPluginIds(messages) {
	const found = /* @__PURE__ */ new Set();
	const pattern = /(?:^|\s)@([a-z]{3,6}-\d+)(?=\s|$)/g;
	for (const message of messages) {
		if (message.source.kind !== "user") continue;
		const text = message.content.flatMap((block) => block.type === "text" ? [block.text] : []).join("\n");
		for (const match of text.matchAll(pattern)) if (match[1] !== void 0) found.add(match[1]);
	}
	return [...found];
}
function renderReference(reference) {
	const mode = reference.currentPackageId === void 0 ? "run" : "update";
	return [
...
function renderUnavailableReference(id) {
	return [
		"<cordis_dynamic_plugin_context>",
